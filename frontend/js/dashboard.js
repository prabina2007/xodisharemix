...
const loadDashboardSongs = async () => {
  ...
  root.innerHTML = songs.length
    ? songs
        .map(
          (song) =>
            `<article class="song-card glass"><img class="song-cover" src="${song.imagePath}" alt="${song.title}"/><div class="song-body"><h4>${song.title}</h4><p class="song-meta">${song.artist}</p><small class="muted">${categoryLabel(song.category)}</small></div></article>`
        )
        .join('')
